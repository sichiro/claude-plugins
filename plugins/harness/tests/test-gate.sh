...
OUT=$(inp "gh pr create --base develop" | sh "$G"); [ -z "$OUT" ] || fail "하네스 미변경인데 deny: $OUT"
...
OUT=$(inp "gh pr create --base develop" | sh "$G"); [ -z "$OUT" ] || fail "OBS·IMP 만 더했는데 deny: $OUT"
# 하네스 변경, 보고서 없음 → deny
...
OUT=$(inp "/opt/homebrew/bin/gh pr merge 7 --squash" | STUB_JSON="$STUB_JSON" PATH="$T/bin:$PATH" sh "$G"); [ -z "$OUT" ] || fail "유효한 merge 인데 deny: $OUT"
...
OUT=$(inp "printf 'gh pr merge docs' > x.md" | sh "$G"); [ -z "$OUT" ] || fail "언급-only(printf)를 막음: $OUT"
OUT=$(inp "git commit -m 'gh pr create 게이트'" | sh "$G"); [ -z "$OUT" ] || fail "언급-only(commit -m)를 막음: $OUT"
OUT=$(inp "gh pr view --json headRefOid" | sh "$G"); [ -z "$OUT" ] || fail "gh pr view 를 막음: $OUT"
...
OUT=$(printf '{"tool_name":"Bash","cwd":"%s","tool_input":{"command":%s}}' "$R" "$(printf '%s' "$CMDH" | jq -Rs .)" | sh "$G"); [ -z "$OUT" ] || fail "heredoc 본문의 호출 줄을 막음: $OUT"
...
# gh 가 아닌 명령 → 통과
OUT=$(inp "git push" | sh "$G"); [ -z "$OUT" ] || fail "무관한 명령을 막음"
# fail-closed
