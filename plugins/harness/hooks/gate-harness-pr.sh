...
printf '%s' "$STRIPPED" | grep -qE -- '(^|[[:space:]])(--repo|-R)([[:space:]=]|$)' && deny "--repo 형태는 지원하지 않는다 — 저장소 체크아웃 안에서 실행한다"
...
if printf '%s' "$STRIPPED" | grep -oE "$INVOKE" | grep -qE 'merge([[:space:]]|$)'; then
  ...
  printf '%s' "$STRIPPED" | grep -qE -- '(^|[[:space:]])(--head|-H)([[:space:]=]|$)' && deny "--head 형태는 지원하지 않는다 — 대상 브랜치를 체크아웃하고 실행한다"
