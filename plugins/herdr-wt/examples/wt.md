...
`wt` 가 `--prompt` 로 첫 지시를 지을 때 이 값으로 채운다. 이슈 본문에서 나오지 않는 것만 여기 둔다.

- **리뷰 행** — 어느 리뷰를 어떤 기준으로 고르는지. 예: 저장·복원·삭제를 건드리면 정확성 리뷰 둘을 함께, 화면만이면 하나
- **검증** — 기본은 로컬·에뮬레이터로 끝난다. 실기기가 필요한 조건은 에이전트 절에 있으니 그 기준을 지시에 옮긴다
- **범위 밖** — 후속 이슈 절을 따른다. 「범위 밖 결함은 이슈로 남기고 이 PR 에 넣지 않는다」
- **끝맺음** — 구현이 끝나면 무엇을 돌리고(리뷰 스킬) PR 본문에 무엇을 넣는지(`Closes #n`)
- **충돌** — 열린 PR 이 같은 파일을 고치면 base 를 어떻게 가져와 푸는지(merge commit 인지 rebase 인지)
