---
name: wt-done
description: 이슈 작업이 끝나 정리할 때 사용한다. "정리해줘" · "마무리해줘" · "머지하고 정리" 지시, PR 을 머지하고 이슈를 닫고 worktree 를 지워야 할 때 쓴다. 되돌릴 수 없는 단계가 섞여 있어 순서가 고정이다
---
...
ls -t ~/.claude/projects/*"$REPO"-*<슬러그>-*/*.jsonl
```
